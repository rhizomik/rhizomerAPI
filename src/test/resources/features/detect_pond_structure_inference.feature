...
Feature: Detect pond structure considering inference
  ...
  Background: Existing pond in local server storing file data
    Given There is a pond "apollo13" on a local server storing "data/nasa-apollo13.ttl" in graph "http://rhizomik.net/pond/apollo13"
    And The following ontologies are defined for the pond "apollo13"
      | data/nasa-schema.ttl                                                            |
    And The query type for pond "apollo13" is "FULL"
    And The inference for pond "apollo13" is set to "true"
